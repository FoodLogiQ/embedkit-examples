...
const ChartRegistry: Array<{ canvas: HTMLCanvasElement; drawFn: () => void }> = []

function debounce<T extends(...a:any[])=>void>(fn:T, ms=120){
  let t: any; return (...a: Parameters<T>) => { clearTimeout(t); t=setTimeout(()=>fn(...a), ms) }
}

function px(n:number, dpr:number) { return Math.round(n*dpr) }
...
function drawGrid(ctx: CanvasRenderingContext2D, x0:number, y0:number, x1:number, y1:number, steps=4) {
  ctx.save()
...
function niceMinMax(values:number[]){
...
export function registerChart(canvas: HTMLCanvasElement, drawFn: () => void) {
...
export function drawLineChart(canvas: HTMLCanvasElement, values:number[]) {
  ...
  const scaleY=(v:number)=> y1 - ((v-min)/(max-min))*(y1-y0)
  ...
  values.forEach((v,i)=>{ const x=x0+i*dx,y=scaleY(v); ctx.beginPath(); ctx.arc(x,y,3,0,Math.PI*2); ctx.fill(); ctx.stroke() })
}
...
export function drawBarChart(canvas: HTMLCanvasElement, labels: string[], values: number[]) {
  const { ctx, w, h } = setCanvasSize(canvas)
...
export function drawDonut(canvas: HTMLCanvasElement, segments: {label:string; value:number; color?:string}[]) {
  const { ctx, w, h } = setCanvasSize(canvas)
  ctx.clearRect(0,0,w,h)
  ...
  ctx.fillStyle='var(--text)'; ctx.font='600 16px system-ui, -apple-system, Segoe UI, Roboto'; ctx.textAlign='center'
  ctx.fillText(`${Math.round((main/total)*100)}%`, cx, cy+6)
}

export function randomSeries(n:number, base=100, spread=25){
